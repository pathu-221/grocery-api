...
export class AuthService {
  ...
  async login(loginCredsDto: LoginDto) {
    ...
    const token = sign({ user: { id: user.id } }, this.configService.get<string>('JWT_SECRET'));
  ...
  async register(userCredsDto: CreateUserDto) {
    const user = await this.userService.findOneBy({
      email: userCredsDto.email,
    });
    if (user) throw new ConflictException('User already Exists');

    const newUser = await this.userService.create(userCredsDto);
    return newUser;
  }
}
